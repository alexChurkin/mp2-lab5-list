...
void TList<T>::PushBack(T element)
{
	...
	//Если список пуст, новое звено - первое
	if (IsEmpty())
		pFirst = newNode;
	//Иначе добавим его в конец списка
	else
	{
		TNode<T>* node = pFirst;
...
void TList<T>::PushAfterMax(T element)
{
	...
	{
		//1. Найдём звено с макс. значением
		TNode<T>* nodeMax = pFirst;

		TNode<T>* node = pFirst;
		while (node->pNext != nullptr)
		{
			if (node->value > nodeMax->value)
				nodeMax = node;

			node = node->pNext;
		}
		//Последняя проверка также необходима
		if (node->value > nodeMax->value)
			nodeMax = node;

		//2. Добавим после него новое звено
		TNode<T>* temp = nodeMax->pNext;
...
T TList<T>::PopBack()
{
	...
	//В списке был 1 элемент - pFirst
	if (prevNode == node)
	{
		delete pFirst;
		pFirst = nullptr;
	}
	//В списке было более 1 элемента
	else
	{
		delete node;
